 ...
 *
 */
public class CollectSend2 extends Verticle {
    ...
    public void start(){
        ...
        bus.registerHandler("splitData.address", new Handler<Message<Character>>() {
            @Override
            public void handle(Message<Character> message) {
                // Hängt gesendete Character von ReadText an batchString an
                batchString.append(message.body()); // Body = <Character> in Zeile 64
                if (batchString.length() >= 1600 / 2) {
                    final String charBuffer = batchString.toString();
